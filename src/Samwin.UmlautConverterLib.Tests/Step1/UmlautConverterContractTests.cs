...
namespace Samwin.UmlautConverterLib.Step1.Tests
{
    public class UmlautConverterContractTests
    {
        ...
        {
            Console.WriteLine($"::group::Step1 Output to inspect : {converter.GetType().Name}.UmlautConversions in {nameof(UmlautConverterContractTests)}.{nameof(Converter_Output)}");
            try
            {
