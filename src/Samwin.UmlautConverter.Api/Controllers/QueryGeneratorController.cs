...
namespace Samwin.UmlautConverter.Api.Controllers
{
    ...
    public class QueryGeneratorController : ControllerBase
    {
        ...
        private async IAsyncEnumerable<SseItem<string>> getQueries([EnumeratorCancellation] CancellationToken clientDisconnectedToken)
        {
            ...
            {
                channel.Writer.TryWrite(msg.ToString());
            }
            ...
            try
            {
                while (true)
                {
                    ...
                    using (_activityService.StartActivity("ReceivedQuery")) ;
                    ...
                    try
                    {
                        ...
                        message = await channel.Reader.ReadAsync(linkedCts.Token);
                        _logger.LogInformation($"Query received and acknowledged: {message}");
                    }
                    ...
                    _logger.LogDebug($"[Log] Sending input to client: {message}");
                    yield return new SseItem<string>($"{message}", eventType: "query");
                }
            }
            finally
            {
